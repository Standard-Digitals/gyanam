...
export default async function AdminStudentsPage({
  ...
  searchParams: Promise<{ page?: string; filter?: string; exam?: string }>;
}) {
  ...
  const where: { id?: { in?: string[]; notIn?: string[] }; targetExam?: string } = {};
  if (filter === 'active') where.id = { in: activeIds };
  if (filter === 'pending') where.id = { notIn: activeIds };
  if (examFilter) where.targetExam = examFilter;
