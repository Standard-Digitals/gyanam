      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white p-5 rounded-2xl border border-[#F3DCDD] shadow-sm space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#FDEAE9] text-[#C12223] flex items-center justify-center shrink-0">
                    <Package className="w-4 h-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-black text-sm text-[#1F1A1C] truncate">{order.orderNumber}</p>
                    <p className="text-[11px] text-[#888888]">{new Date(order.createdAt).toLocaleString()}</p>
                  </div>
                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase shrink-0 ${STATUS_COLORS[order.orderStatus] ?? 'bg-gray-100 text-gray-700'}`}>
                    {order.orderStatus}
                  </span>
                </div>
                <div className="space-y-1.5 border-t border-[#F3DCDD] pt-3">
                  {items.map((item, idx) => (
                ...
                <div className="flex justify-between items-center text-sm font-black text-[#C12223] border-t border-[#F3DCDD] pt-3">
                  <span className="font-plexmono text-[11px] uppercase text-[#888888] font-bold">
                    Total ({order.paymentMethod === 'cod' ? 'COD' : order.paymentMethod})
                  </span>
