...
import { CheckCircle2, Circle, ChevronLeft, ChevronRight, ArrowLeft, PlayCircle, Clock, ClipboardList, Play } from 'lucide-react';
...
interface CourseQuiz extends QuizTakerQuiz {
  difficulty: string;
}
  ...
  if (activeQuiz) {
    return (
      <div className="space-y-4">
        <QuizTaker quiz={activeQuiz} onExit={() => setActiveQuiz(null)} />
      </div>
    );
  }
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
              tab === 'lessons' ? 'bg-white text-[#1F1A1C] border border-[#F3DCDD] shadow-sm' : 'text-[#8A7A7B] hover:text-[#1F1A1C]'
            }`}
          >
            Lessons
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
              tab === 'quizzes' ? 'bg-white text-[#1F1A1C] border border-[#F3DCDD] shadow-sm' : 'text-[#8A7A7B] hover:text-[#1F1A1C]'
            }`}
          >
          ...
          {quizzes.map((quiz) => (
            <div key={quiz.id} className="bg-white p-5 rounded-2xl border border-[#F3DCDD] shadow-sm space-y-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-heading font-black text-sm text-[#1F1A1C]">{quiz.title}</h3>
                  <p className="text-[11px] text-[#888888]">{quiz.totalQuestions} questions · {quiz.timeLimitMinutes} mins · {quiz.difficulty}</p>
                </div>
              </div>
              <button
